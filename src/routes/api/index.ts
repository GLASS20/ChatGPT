...
export async function fetchBilling(key: string): Promise<Billing> {
  ...
  try {
    ...
    const urlSubscription =
      "https://api.chatanywhere.com.cn/v1/dashboard/billing/subscription" // 查是否订阅
    const urlUsage = `https://api.chatanywhere.com.cn/v1/dashboard/billing/usage?start_date=${formatDate(
      startDate
